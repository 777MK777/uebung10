...
public class Marketing {
    ...
    public Marketing(KundenRepository kundenRepository,
        ...
        this.kundenRepository = Objects.requireNonNull(kundenRepository);
        this.mailVersand = Objects.requireNonNull(mailVersand);
        this.marketingCore = Objects.requireNonNull(marketingCore);
        this.zufall = Objects.requireNonNull(zufall);
    }
    ...
    void sendMails(List<Kunde> kunden) {
        for (Kunde k : kunden) {
            if (k.darfMailEmpfangen() && zufall.fuerMailAusgewaehlt()) {
                sendMail(k);
            }
        }
    }
    ...
    void sendMail(Kunde k) {
        ...
        Mail mail = marketingCore.mailErstellen(k.name());
